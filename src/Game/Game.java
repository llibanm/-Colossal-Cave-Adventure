...
public class Game //extends CommandImplementation.CommandImplementation
{
    ...
    public void setHero(){
        ...
        System.out.println("\nIt's time to go.");
        //PrintMessageWithHeroName("Welcome to the Game2 :");
    }
    ...
    public void transformStringToEnum(String command){
        switch(command.toUpperCase()){ // on le change en type énuméré
            ...
            case "SETPIN":
                currentCommand= EnumCommand.Command.SETPIN;break;
            default:
                System.out.println("""
                        Unknown command.
                        please try again.
    ...
    public void addCommandArgumentIntoList() {
        String[] parts = commandString.split(" ");
        if (parts.length == 0) {
            return;
        }

        transformStringToEnum(parts[0]);

        for (int i = 1; i < parts.length; i++) {
            command.addCommand(parts[i]);
        }
    }
    ...
    public void unlockExitWithPin(int pin, String exit){
        ...
        Exit e = getCurrentLocation().getExit(exit);
    ...
    public void unlockExitWithKey(Key key, String exit){
        Exit e = getCurrentLocation().getExit(exit);
